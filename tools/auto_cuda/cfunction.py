...
def decay_add(z: str or None, x: str, y: str, w: str, dtype: str):
    if dtype == 'float':
        if x == '0.0f':
            codes = f'{w} * {y}'
        elif y == '0.0f':
            ...
            codes = f'(1.0f - {w}) * {x} + {w} * {y}'
    elif dtype == 'half2':
        if x == '__float2half2_rn(0.0f)':
            codes = f'__hmul2({w}, {y})'
        elif y == '__float2half2_rn(0.0f)':
            codes = f'__hmul2(__hsub2(__float2half2_rn(1.0f), {w}), {x})'
        else:
            codes = f'__hadd2(__hmul2(__hsub2(__float2half2_rn(1.0f), {w}), {x}), __hmul2({w}, {y}))'
    else:
        raise NotImplementedError(dtype)
    return wrap_return_codes(z, codes)


# v = v + (x + a0 * (v - vrest) * (v - vc)) * (1 - w)
def qif_decay_add(z: str or None, x: str, y: str, tau: str, a0: str, vrest: str, vc: str, dtype: str):  # x -> x, y -> v
    if dtype == 'float':
        if x == '0.0f':
            codes = f'{y} + {a0} * ({y} - {vrest}) * ({y} - {vc}) / {tau}'
        elif y == '0.0f':
            codes = f'{x} / {tau}'
        else:
            codes = f'{y} +  {x} / {tau} + {a0} * ({y} - {vrest}) * ({y} - {vc}) / {tau}'
    else:
        raise NotImplementedError(dtype)
    return wrap_return_codes(z, codes)


def fc_decay_add(z: str or None, x: str, y: str, i: str, alpha: str, fc: str, dtype: str):  # z='i', x='x_seq[t]', y='spike_seq[t-1]', alpha='alpha', fc='fc', dtype=self.dtype
    if dtype == 'float':  # i[t] = alpha * i[t-1] + x + fc * y
        if x == '0.0f':
            codes = f'{alpha} * {i} + {fc} * {y}'
        elif y == '0.0f':
            codes = f'{alpha} * {i} + {x}'
        elif i == '0.0f':
            codes = f'{x} + {fc} * {y}'
        else:
            codes = f'{alpha} * {i} + {x} + {fc} * {y}'
    elif dtype == 'half2':
        if x == '__float2half2_rn(0.0f)':
            codes = f'__hadd2(__hmul2({alpha}, {i}), __hmul2({fc}, {y}))'
        elif y == '__float2half2_rn(0.0f)':
            codes = f'__hadd2(__hmul2({alpha}, {i}), {x})'
        elif i == '__float2half2_rn(0.0f)':
            codes = f'__hadd2({x}, __hmul2({fc}, {y}))'
        else:
            codes = f'__hadd2(__hadd2(__hmul2({alpha}, {i}), __hmul2({fc}, {y})), {x})'
    else:
        raise NotImplementedError(dtype)
    return wrap_return_codes(z, codes)
...
def equal(y: str or None, x: str, dtype: str):
    if dtype == 'float':
        codes = f'{x}'
    elif dtype == 'half2':
        codes = f'{x}'
...
def qif_h_v_grad(z: str or None, tau: str, v: str, a0: str, vrest: str, vc: str, dtype: str):
    if dtype == 'float':
        codes = f'1.0f + {a0} * (2.0f * {v} - {vc} -{vrest}) / {tau}'
    else:
        raise NotImplementedError(dtype)
    return wrap_return_codes(z, codes)
...
def sigmoid2(y: str or None, x: str, dtype: str):
    if dtype == 'fp32':
        codes = f'1.0f / (1.0f + expf(-4 * {x}))'
    elif dtype == 'fp16':
        codes = f'__h2div(__float2half2_rn(1.0f), __hadd2(__float2half2_rn(1.0f), h2exp(__hneg2(__hmul2(4, {x})))))'
    else:
        raise NotImplementedError(dtype)
    return f'''
    const float {y} = {codes};
